...
import {
  formatDatabaseList,
  parseDatabaseList,
  type PostgresDatabaseMode,
} from "@/lib/discovery/postgres/databases";
import { cn } from "@/lib/utils";

export function PostgresDatabasePicker({
  values,
  availableDatabases,
  loading,
  error,
  ...
  const mode: PostgresDatabaseMode =
    values.databaseMode === "all" ? "all" : "selected";
  const selected = new Set(parseDatabaseList(values.databases));
  const hasList = availableDatabases.length > 0;

  function setMode(next: PostgresDatabaseMode) {
    if (next === "all") {
      onChange({
        databaseMode: "all",
        databases: formatDatabaseList(availableDatabases),
        database: availableDatabases[0] ?? values.database ?? "postgres",
      });
      return;
    }
    onChange({
          ...
          [
            {
              value: "all" as const,
              label: "All databases",
              detail: "Scan every non-template database visible to this role.",
            },
            {
...
export function isPostgresDatabaseSelectionReady(
  values: Record<string, string>,
): boolean {
