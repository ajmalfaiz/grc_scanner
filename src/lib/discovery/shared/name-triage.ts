...
/**
 * Shared metadata name-triage engine — column names (Postgres/MySQL) and
 * field paths (MongoDB/SaaS) are triaged with the same rule set so PII
 * naming conventions (aadhaar, pan, email, …) are recognised consistently
 * across every connector. Never reads row/document/cell values.
 */
...
export type NameTriageHit<T = ColumnLike> = {
  column: T;
  piiType: string;
...
export const RULES: Rule[] = [
  ...
  },
  {
    ...
    dataTypes: [/text|character|varchar|json|string|object/i],
  },
  {
    piiType: "Person name",
    confidence: "medium",
...
export const FREETEXT_TYPES =
  /^(text|character varying|varchar|json|jsonb|citext|string|object|mixed)$/i;

/**
 * Core matcher: a bare `name` (column/field), a blob for context patterns
 * (schema/table/path), and an optional type blob. No row/cell values.
 */
export function matchName(
  ...
  for (const rule of RULES) {
    ...
    if (rule.negativePatterns?.some((pattern) => pattern.test(contextBlob))) {
      continue;
    }
    if (
      rule.dataTypes &&
      typeBlob &&
      !rule.dataTypes.some((pattern) => pattern.test(typeBlob))
    ) {
      continue;
    }
    const contextMatch = rule.contextPatterns?.some((pattern) =>
      pattern.test(contextBlob),
    );
    const score =
      60 + (rule.confidence === "high" ? 20 : 0) + (contextMatch ? 15 : 0);
    const hit = {
      ...
      signals: (contextMatch
        ...
        : ["metadata_name"]) as DetectionSignal[],
      reasons: [
        `Column/field name matched ${rule.piiType} triage rule`,
        ...(contextMatch
          ? [`Context strengthened ${rule.piiType} signal`]
          : []),
      ],
    };
...
export function isFreeTextType(dataType: string, udtName?: string): boolean {
  return FREETEXT_TYPES.test(dataType) || (!!udtName && FREETEXT_TYPES.test(udtName));
}

export function triageColumn<T extends ColumnLike>(column: T): NameTriageHit<T> | null {
  const contextBlob = `${column.schema} ${column.table} ${column.column}`;
  const typeBlob = `${column.dataType} ${column.udtName}`;
  const match = matchName(column.column, contextBlob, typeBlob);
  if (!match) return null;
  return { column, ...match };
}

export function triageColumns<T extends ColumnLike>(columns: T[]): NameTriageHit<T>[] {
  const hits: NameTriageHit<T>[] = [];
...
/** Triage a nested document/JSON field path (e.g. `profile.contact.email`). */
export function triageFieldPath(
  fieldPath: string,
  contextBlob?: string,
): FieldTriageHit | null {
  const leaf = fieldPath.split(".").pop() ?? fieldPath;
  const match = matchName(leaf, contextBlob ?? fieldPath);
  if (!match) return null;
  return { fieldPath, ...match };
}

function mergeTriageHits<T extends ColumnLike>(
  hits: NameTriageHit<T>[],
): NameTriageHit<T>[] {
  const byKey = new Map<string, NameTriageHit<T>>();
  for (const hit of hits) {
    ...
    if (!existing || (hit.score ?? 0) > (existing.score ?? 0)) {
      byKey.set(key, {
        ...
        signals: [...new Set([...(existing?.signals ?? []), ...(hit.signals ?? [])])],
      });
    } else {
      byKey.set(key, {
        ...existing,
...
function geminiPrompt<T extends ColumnLike>(columns: T[]): string {
  return [
...
async function fetchGeminiTriage<T extends ColumnLike>(
  columns: T[],
): Promise<NameTriageHit<T>[]> {
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [{ role: "user", parts: [{ text: geminiPrompt(columns) }] }],
...
export async function triageColumnsWithOptionalLlm<T extends ColumnLike>(
  columns: T[],
  mode: string | undefined,
): Promise<{ hits: NameTriageHit<T>[]; llmAssistUsed: boolean }> {
